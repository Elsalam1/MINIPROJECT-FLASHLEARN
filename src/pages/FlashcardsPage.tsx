...
const FlashcardsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const savedFlashcards = await loadData('flashlearn-flashcards');
        if (savedFlashcards) {
  ...
  const handleGenerateFlashcards = async () => {
    ...
    try {
      ...
      await saveData('flashlearn-flashcards', updatedFlashcards);
      // Add to recent activity
      const activity = JSON.parse(localStorage.getItem('flashlearn-recent-activity') || '[]');
      activity.unshift({ type: 'flashcard', title: `Generated ${newFlashcards.length} flashcards from "${selectedNote.title}"`, date: new Date().toISOString() });
  ...
  const handleDeleteFlashcard = async (id: string) => {
    ...
    // Add to recent activity
    const activity = JSON.parse(localStorage.getItem('flashlearn-recent-activity') || '[]');
    activity.unshift({ type: 'flashcard', title: 'Deleted a flashcard', date: new Date().toISOString() });
    localStorage.setItem('flashlearn-recent-activity', JSON.stringify(activity.slice(0, 10)));
  };
  ...
  // Repetitive learning mode state
  const [repetitiveMode, setRepetitiveMode] = useState<boolean>(false);
    ...
    <div className="flashcards-page">
      <div className="flashcards-header">
        ...
        <div className="flashcards-controls">
          ...
          <button
            className="repetitive-mode-btn"
            onClick={() => setRepetitiveMode(!repetitiveMode)}
            disabled={flashcards.length === 0}
          >
            {repetitiveMode ? 'Exit Repetitive Mode' : 'Repetitive Learning Mode'}
          </button>
        </div>
      </div>
      ...
      {flashcards.length === 0 ? (
        ...
        repetitiveMode ? (
          <div className="flashcards-repetitive-container">
            <FlashCard front="" back="" cards={flashcards.map(card => ({ front: card.front, back: card.back }))} />
          </div>
        ) : (
