...
const NotesPage: React.FC = () => {
  ...
  const handleSaveNote = useCallback(async (content: string, title: string) => {
    ...
    if (selectedNote) {
      ...
      await saveData('flashlearn-notes', updatedNotes);
      // Add to recent activity
      const activity = JSON.parse(localStorage.getItem('flashlearn-recent-activity') || '[]');
      activity.unshift({ type: 'note', title, date: now.toISOString() });
      ...
      await saveData('flashlearn-notes', newNotes);
      // Add to recent activity
      const activity = JSON.parse(localStorage.getItem('flashlearn-recent-activity') || '[]');
      activity.unshift({ type: 'note', title: newNote.title, date: now.toISOString() });
      localStorage.setItem('flashlearn-recent-activity', JSON.stringify(activity.slice(0, 10)));
    }
  ...
  const handleAddNote = useCallback(() => {
    ...
    setNotes(newNotes);
    setSelectedNote(newNote);
    saveData('flashlearn-notes', newNotes);
    // Add to recent activity
    const activity = JSON.parse(localStorage.getItem('flashlearn-recent-activity') || '[]');
    activity.unshift({ type: 'note', title: newNote.title, date: now.toISOString() });
    localStorage.setItem('flashlearn-recent-activity', JSON.stringify(activity.slice(0, 10)));
  ...
  const handleDeleteNote = useCallback(async (id: string) => {
    ...
    // Add to recent activity
    const activity = JSON.parse(localStorage.getItem('flashlearn-recent-activity') || '[]');
    activity.unshift({ type: 'note', title: 'Deleted Note', date: new Date().toISOString() });
    localStorage.setItem('flashlearn-recent-activity', JSON.stringify(activity.slice(0, 10)));
  }, [notes, selectedNote, saveData]);
